...
void multiplyNumbersInPattern(const string &text, const regex &pattern, vector<int> &products,bool mulActive) {
    auto matchesBegin = sregex_iterator(text.begin(), text.end(), pattern);
    auto matchesEnd = sregex_iterator();
    const regex pattern2 = "do()";
    const regex pattern2 = "don't()";
